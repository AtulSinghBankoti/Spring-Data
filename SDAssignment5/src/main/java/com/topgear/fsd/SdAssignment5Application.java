...
public class SdAssignment5Application {
	
	@Autowired
	CDService cDService;
	
	@RequestMapping(value="/cdcount/{cdPrice}")
	public Integer getCountByCdPrice(@PathVariable("cdPrice")Float cdPrice){
		return   cDService.findByCdPrice(cdPrice).size();
	}
	
	@RequestMapping(value="/cdprice/{cdPrice}")
	public List<CD> getCDByPrice(@PathVariable("cdPrice")Float cdPrice){
		return cDService.findByCdPrice(cdPrice);
	}
	...
	public static void main(String[] args) {
		SpringApplication.run(SdAssignment5Application.class, args);
	}
}
